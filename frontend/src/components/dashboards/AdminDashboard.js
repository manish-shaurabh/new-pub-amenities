...
import {
  Box, BarChart3, Building2, ChevronDown, ArrowRight, Filter, X, Wrench, ShieldAlert,
} from 'lucide-react';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setReportingOfficers((u.data || []).filter((x) => x.role === 'reporting_officer' && x.is_active !== false));
